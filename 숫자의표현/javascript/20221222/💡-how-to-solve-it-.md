...
n이라는 숫자가 주어질 때 연속된 자연수로 n을 표현하는 방법의 수를 반환하는 함수를 작성

### 1. 이해
- 숫자는 10,000이하 자연수다
- 연속된 자연수로 n을 표현해야 한다
- 자연수는 1부터 시작하고 최대 크기는 n이다

### 2. 계획
- 1부터 n까지 1씩 추가해서 n이 나오는 함수를 작성
- n보다 클 경우 초기값 +1, n과 같을 경우 초기값 +1 과 result + 1,
- 과정을 Math.ceil(n / 2)까지 반복
